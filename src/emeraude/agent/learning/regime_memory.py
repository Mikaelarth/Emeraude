...
class RegimeStats:
    ...
    Stored fields (raw) :
        n_trades, n_wins, sum_r (sum of R-multiples), sum_r2 (sum of
        squared R-multiples for variance).
...
class RegimeMemory:
    ...
    def record_outcome(self, strategy: str, regime: Regime, r_multiple: Decimal) -> None:
        ...
        with database.transaction() as conn:
            row = conn.execute(
                "SELECT n_trades, n_wins, sum_r, sum_r2 FROM regime_memory "
            ...
            if row is None:
                conn.execute(
                    ...
                    "(strategy, regime, n_trades, n_wins, sum_r, sum_r2) "
                    "VALUES (?, ?, 1, ?, ?, ?)",
                    (
                        strategy,
                        regime.value,
    ...
    def get_stats(self, strategy: str, regime: Regime) -> RegimeStats:
        ...
        if row is None:
            return RegimeStats(n_trades=0, n_wins=0, sum_r=_ZERO, sum_r2=_ZERO)
        return RegimeStats(
